...
!   call fld_list_add(fldsFrHyd, "water_surface_height_above_reference_datum"  , "cannot provide", "conservefrac")
  ...
  subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
    ...
    call ESMF_AttributeGet(gcomp, name="DumpRHs", value=value, defaultValue="true", &
                           convention="NUOPC", purpose="Instance", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!!ssun #ifndef FRONT_FV3
!!ssun           call ESMF_FieldWrite(fieldAtmMask,'field_med_atm_a_land_mask.nc',overwrite=overwrite_flag,rc=rc)
!!ssun           if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,line=__LINE__, file=__FILE__)) return
!!ssun           write (msgString,*) trim(subname)//"land_mask = ",minval(dataPtr_fieldAtm),maxval(dataPtr_fieldAtm)
!!ssun           call ESMF_LogWrite(msgString, ESMF_LOGMSG_INFO, rc=rc)
!!ssun #endif
          ! write out masks
!===========
! -- Create ESMFIO
